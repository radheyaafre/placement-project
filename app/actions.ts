    ...
    <div style="margin:0;padding:28px 16px;background:#0b1016;color:#f3f5f7;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
      <div style="max-width:680px;margin:0 auto;border:1px solid rgba(255,255,255,0.08);border-radius:24px;overflow:hidden;background:#151b23;">
        <div style="padding:28px;background:linear-gradient(180deg,#1b2432 0%,#151b23 100%);border-bottom:1px solid rgba(255,255,255,0.08);">
          ...
          <h1 style="margin:12px 0 8px;font-size:28px;line-height:1.1;color:#f8fafc;">Placement Prep Update</h1>
          <p style="margin:0;color:#cbd5e1;line-height:1.6;">
            A quick note from the SamyakLabs.AI admin team for your placement preparation journey.
          </p>
        </div>
        ...
        <div style="padding:28px;display:grid;gap:18px;">
          <div style="padding:18px;border-radius:18px;border:1px solid rgba(255,184,77,0.16);background:rgba(255,161,22,0.08);">
            <div style="font-size:13px;color:#ffcf7a;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;">Message</div>
            <div style="margin:12px 0 0;color:#e5e7eb;line-height:1.8;font-size:15px;">${safeMessage}</div>
          </div>

          <div style="display:grid;gap:12px;">
            <div style="padding:16px;border-radius:18px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);">
              <div style="font-size:12px;color:#9aa4b2;text-transform:uppercase;letter-spacing:0.06em;">Next step</div>
              <p style="margin:8px 0 0;color:#f8fafc;line-height:1.7;">
                Log in to your dashboard, continue your pending tasks, and keep your preparation rhythm steady.
              </p>
            </div>
          </div>
          ...
          <div>
            <a
              href="${dashboardUrl}"
              style="display:inline-flex;align-items:center;justify-content:center;min-height:46px;padding:0 18px;border-radius:12px;background:linear-gradient(180deg,#ffb84c,#ffa116);color:#1a1207;text-decoration:none;font-weight:800;"
            >
        ...
        <div style="padding:18px 28px;border-top:1px solid rgba(255,255,255,0.08);color:#9aa4b2;font-size:13px;line-height:1.7;">
          Sent from SamyakLabs.AI admin console. If you already completed your current task, keep going with the next available step in your dashboard.
        </div>
      </div>
    </div>
  `;
...
export async function sendAdminBroadcastAction(
  ...
  try {
    ...
    const text = `${message}\n\nOpen your placement portal: ${getAppUrl()}/dashboard\n\nSent from SamyakLabs.AI admin console.`;
    const html = buildAdminBroadcastHtml(message);
...
export async function requestPasswordResetAction(formData: FormData) {
  ...
  const { error } = await supabase.auth.resetPasswordForEmail(email, {
    redirectTo: `${getAppUrl()}${buildRedirect("/auth/confirm", {
      next: "/reset-password"
    })}`
  });
